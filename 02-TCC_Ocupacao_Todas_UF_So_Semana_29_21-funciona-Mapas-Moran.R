...
composicao_por_municipios_spark <- copy_to(sc, composicao_por_municipios,name = "composicao_por_municipios_spark", overwrite = TRUE)
...
ocupacao <- sparklyr::left_join(x = ocupacao_carregada, y = composicao_por_municipios_spark, by = join_by(CO_MUN_RES == code_muni))
...
# Filtra apenas estado do Rio de Janeiro
# spatial_regiao_imediata <- spatial_regiao_imediata[(substr(spatial_regiao_imediata$rgi,1,2)==33),]
...
moran.test(x = spatial_regiao_imediata@data$ocorrencias, 
           listw = listw_queen, 
           zero.policy = TRUE
...
moran.plot(x = spatial_regiao_imediata@data$ocorrencias, 
           listw = listw_queen, 
           zero.policy = TRUE,
           xlab = "Ocorrencias", 
...
moran_local <- localmoran(x = spatial_regiao_imediata@data$ocorrencias, 
                          listw = listw_queen, 
                          zero.policy = TRUE)
...
# redor de sua média:
centro_idh <- spatial_regiao_imediata@data$ocorrencias - mean(spatial_regiao_imediata@data$ocorrencias)
